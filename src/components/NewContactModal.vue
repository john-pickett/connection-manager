  ...
  <v-layout row justify-center>
    <v-dialog v-model="dialog" persistent scrollable max-width="800px">
      <v-card>
            ...
            <v-container>
                ...
                <v-layout row wrap>
                    <v-flex xs12 sm6 md6 class="pa-1">
                        <v-text-field label="First Name" v-model="contact.firstName" autofocus></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6 md6 class="pa-1">
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" flat @click="cancel">Close</v-btn>
          <v-btn color="success" flat @click="saveNewContact">Save</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-layout>
...
export default {
    ...
    methods: {
        cancel () {
            this.$store.state.newContactModal = false;
            this.clearForm();
        },
        saveNewContact () {
        ...
        menuClicker (e, item) { // for choosing the contact's status
    ...
    computed: {
        dialog () {
            return this.$store.state.newContactModal;
        }
    }
}
</script>
